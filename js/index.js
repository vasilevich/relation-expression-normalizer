 ...
 * @param expression
 */
const removeEnclosures = (expression) => {
 ...
 * @param endChar
 */
const findGroups = (expression, startChar = '[', endChar = ']') => {
 ...
 * @param expression
 */
const replaceEnclosureWithKeys = (expression) => {
 ...
 * @param replacedObj
 */
const replaceKeysWithEnclosures = ({ expression, replacedObj }) => {
 ...
 * @param delimeter
 */
const safeSplitChar = (expression, delimeter) => {
    expression = removeEnclosures(expression).replace(/ /g, '');
 ...
 * @param expression
 */
const safeSplitComma = (expression) => safeSplitChar(expression, ',');
/**
 * safe split by dot
 * @param expression
 */
const safeSplitDot = (expression) => safeSplitChar(expression, '.');
/**
 * get recursive obj using keys from an array
 * @param arr
 * @param keysGatherer
 */
const getObjByArray = (arr, keysGatherer) => {
 ...
 * @param sourceObj
 */
const recursivelyMerge = (destinationObj, sourceObj) => {
 ...
 * @param depth
 */
const parseExpression = (expression, inputArr = [], depth = 0) => {
    ...
    for (const subSection of subSections) {
        ...
        for (const subSubSection of subSection.slice(1)) {
            if (subSubSection.includes(',') || subSubSection.includes('.')) {
                const subSubSectionKeysGatherer = exports.parseExpression(subSubSection, arr, depth + 1);
 ...
 * @param expressionObj
 */
const stringifyExpression = (expressionObj) => {
    ...
    for (const key of keys) {
        ...
        if (isDeeper) {
            keyResult.push(`${key}.${exports.stringifyExpression(value)}`);
        }
        else {
            keyResult.push(key);
        }
 ...
 * @param expression
 */
const normalizeRelationExpression = (expression) => exports.stringifyExpression(exports.parseExpression(expression));
exports.default = normalizeRelationExpression;
